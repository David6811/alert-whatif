...
export const copyFilePatterns = [
  ...
  { from: 'plugin.json', to: '.' },
  // Repo lives in a pnpm monorepo: LICENSE is at the workspace root.
  // Webpack's `context` is the plugin's `src/`, so `../../../` walks up
  // src → plugin → packages → workspace-root. CHANGELOG is local to the
  // plugin and stays at `../`.
  { from: '../../../LICENSE', to: '.' },
